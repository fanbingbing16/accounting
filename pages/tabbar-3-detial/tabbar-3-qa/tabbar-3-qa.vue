		...
		<view class="keyboard">
			...
			<view class="box flex">
				<view class="box-item" v-for="(item,index) in buttonLists" :key="item.text" @click="butonClick(item)"
					...
					<text
						class="text">{{item.id===1?(isTodayF(selectTime)?'今天':transformMonthDateF(selectTime)):item.text}}</text>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		setup() {
			...
			function transformMonthDateF(date) {
				return transformMonthDate(date)
			}
