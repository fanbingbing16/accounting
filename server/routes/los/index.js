...
router.post('/addEdit', (req, res) => {
  ...
  db.query('select * from user where id=111111111111 and password=' + password, [], res, function (result, fields) {
    if (result.length === 1) {
      if (!id) {
        db.query('insert into logs(name,id,content,update_time,type1,type2,isMarkdown) values(?,?,?,?,?,?,?)', [name, getId(), content, new Date(), type1, type2, isMarkdown], res, function (result, fields) {
          res.send({ status: 1, msg: '新增成功', data: null })

        })
      } else {
        db.query('update logs set name=?,content=?,update_time=?,type1=?,type2=?,isMarkdown=? where id=?', [name, content, new Date(), type1, type2, isMarkdown, id], res, function (result, fields) {
